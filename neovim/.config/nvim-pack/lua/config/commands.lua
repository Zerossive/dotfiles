...
-- end, { nargs = 1|*|?|+, range = true|false, complete = ..., desc = description }) -- number of expected arguments
...
vim.api.nvim_create_user_command('RC', function(opts)
	...
	local result = vim.fn.system({ 'sh', '-c', opts.args }, input)
	print(result)
end, {
	desc = 'Run code on selected text and print result',
	range = true,
	nargs = '+',
	complete = 'shellcmd',
})
